...
def select(id):
    ...
    if results:
        result = results[0]
        manufacture = manufacture_repository.select(result['manufacture_id'])
...
def update(product):
    sql = """
    UPDATE products SET 
    (model, description, stock_count, trade_price, sale_price, manufacture_id, type_id)
    = (%s, %s, %s, %s, %s, %s, %s)
    WHERE id = %s
    """
    values = [product.model, product.description, product.stock_count, product.trade_price, product.sale_price, product.manufacture.id, product.type.id, product.id]
    run_sql(sql, values)
